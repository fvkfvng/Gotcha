<?php
    ...
    if (isset($_GET['id'])) {
        ...
        $query  = "SELECT * FROM tbl_point
                    INNER JOIN tbl_user_card ON tbl_point.p_from = tbl_user_card.uc_id
                    INNER JOIN tbl_user ON tbl_user_card.uc_user_id = tbl_user.u_id
                    WHERE p_id = '".$id."'";
        $result = mysqli_query($conn, $query) or die(mysql_error());
        $row    = $result->fetch_assoc();

        $query_tr  = "SELECT * FROM tbl_point
                    INNER JOIN tbl_user_card ON tbl_point.p_receive = tbl_user_card.uc_id
                    INNER JOIN tbl_user ON tbl_user_card.uc_user_id = tbl_user.u_id
                    WHERE p_id = '".$id."'";
        $result_tr = mysqli_query($conn, $query_tr) or die(mysql_error());
                ...
                <div class="block-success-body">
                    <div class="block-success-row">
                        <div class="block-success-left">
                            <div class="profile-picture mb-10">
                    ...
                    <div class="block-success-row">
                        <div class="block-success-left">
                            <div class="profile-picture">
                                <img src="<?php echo $row_tr['u_line_img']; ?>" />
                            </div>
                        </div>
                        <div class="block-success-right">
                            <?php echo $row_tr['u_name']; ?><br>
                            <?php echo $row_tr['u_tel']; ?>
                        </div>
                    </div>
                </div>
            </div>
